...
function prettyPrintTruthTable(parseResult) {
	...
	generateTruthTable(parseResult, outputRow(tbody));
...
function createTableElement() {
	...
	result.className = "table table-bordered container-fluid text-center";
	return result;
}
...
function outputRow(table) {
	return function(assignment, result) {
		...
		for (var i = 0; i < assignment.length; i++) {
			...
			cell.innerHTML = (assignment[i]? "True" : "False");
			row.appendChild(cell);
		}
		
		/* Show the value of the expression. */
		var lastCell = document.createElement("td");
		lastCell.innerHTML = (result? "True" : "False");
...
function showObject(object) {
	...
	var target = document.getElementsByClassName('table-container')[0];
